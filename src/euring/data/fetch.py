...
def _fetch_places_csv(url: str) -> list[dict[str, object]]:
    ...
    for row in rows:
        ...
        if len(trimmed) >= 6:
            ...
            updated = trimmed[-1]
            # Catch real world malformatted code tables where notes have pipe characters.
            # We suppose that these are originally commas and thus revert them to commas.
...
def write_code_table_files(output_dir: str, datasets: dict[str, Iterable[dict[str, object]]]) -> None:
    """Write reference datasets to Python code table files in the target directory."""
    os.makedirs(output_dir, exist_ok=True)
    for filename, data in datasets.items():
        base = os.path.splitext(filename)[0]
        module_name = f"code_table_{base}.py"
        path = os.path.join(output_dir, module_name)
        table = _normalize_dates(list(data))
        content = _render_code_table(module_name, table)
        with open(path, "w", encoding="utf-8") as code_file:
            code_file.write(content)


def _normalize_dates(data: list[dict[str, object]]) -> list[dict[str, object]]:
...
def _render_code_table(module_name: str, table: list[dict[str, object]]) -> str:
    import pprint

    header = [
        "from __future__ import annotations",
        "",
        f"# Generated by euring.data.fetch ({module_name}).",
        "TABLE = " + pprint.pformat(table, width=120, sort_dicts=True),
        "",
        '__all__ = ["TABLE"]',
        "",
    ]
...
def main() -> None:
    ...
    datasets = fetch_all()
    write_code_table_files(args.output_dir, datasets)
